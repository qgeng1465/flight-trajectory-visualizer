...
> **▶ 演示视频（35 秒）：** [**在浏览器里播放 →**](https://qgeng1465.github.io/flight-trajectory-visualizer/demo.html) — MP4（2.8 MB）+ WebM（3.3 MB），1280×760，点封面图同样可以打开。收录了导入日志、飞机沿高度剖面飞行、行程回放，以及按评分排序的机场搜索。文件本身是 [`demo.mp4`](demo.mp4) 和 [`demo.webm`](demo.webm)；上面的链接打开的是一个带真正播放器的页面。
